...
/**
 * SPOJ 346 - Bytelandian gold coins Created by Darren on 14-7-16. Optimal
 * strategy: when n < 12, we exchange it directly for USD; when n >= 12, we
 * always make exchange with the bank. Observation: we won't get better off by
 * making exchange with the bank when n < 12, or get worse off by making
 * exchange with the bank when n >= 12. The above observation can be proved by
 * strong induction. For the second part of the observation, the base cases are
 * when n = 12, ..., 23. n = 12 + x, x >= 12 n/2+n/3+n/4 = 13+(x/2+x/3+x/4) >=
 * 13+x > n
 */
public class Main {
